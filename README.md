...
下载该主题后，打开 `偏好设置→外观→主题→打开主题文件夹`。之后将解压后的 `css` 文件以及 `assets` 和`FONT`文件夹复制到该文件夹下，重新进入 `偏好设置→外观→主题` 选择喜欢主题后，重启 Typora 即可。
...
> + `user-theme` 可以快速地调整淡色主题，`user-dark-theme `可以快速地调整暗色主题。
...
+ 内置了所需的英文字体、中文字体（黑体、宋体、楷体）。

## 关于 Print 主题的使用

+ 该主题依然处于待完善状态。有打印、导出需求的可以使用 VS Code 下的插件。详见
